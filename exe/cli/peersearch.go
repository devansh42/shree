...
func managePeerDiscoveryListener(discoveredPeer chan peerInfo) error {
	listener, err := net.Listen("udp", exe.JoinHost("", shreePort))
	...
	for {
		...
		case STATE_0:
			...
			case hELLO: //Let's send response back
				...
				p.Addr, _ = net.ResolveUDPAddr("udp4", exe.JoinHost("", shreePort)) //Server's address
...
func managePeerDiscoveryResponder() {
	for _, v := range getBroadcastAddress() {
		conn, err := net.Dial("udp", exe.JoinHost(v, shreePort))
		if err != nil {
